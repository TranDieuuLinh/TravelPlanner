...
## Admin AI Knowledge Graph imports

Các endpoint này yêu cầu role `admin`; mọi mutation yêu cầu CSRF. AI chỉ tạo
proposal để review, không ghi thẳng knowledge graph.

### Tạo proposal

```http
POST /api/admin/knowledge-graph/imports
```

Request gồm `sourceLabel`, `sourceUrl` tùy chọn và `content` từ 20 đến 50.000 ký
tự. Response gồm node/edge, evidence, confidence, match status, candidates,
validation issues và decision hiện tại. Mỗi node còn trả `requiredProperties`
và `optionalProperties` đã resolve toàn bộ chuỗi kế thừa trong `schema.yaml` để
Admin Frontend dựng đúng các trường nhập liệu theo loại node.

### Review và apply

```http
GET  /api/admin/knowledge-graph/imports
GET  /api/admin/knowledge-graph/imports/{importId}
PUT  /api/admin/knowledge-graph/imports/{importId}/nodes/{tempId}
PUT  /api/admin/knowledge-graph/imports/{importId}/edges/{tempId}
POST /api/admin/knowledge-graph/imports/{importId}/revalidate
POST /api/admin/knowledge-graph/imports/{importId}/apply
```

Decision gồm `pending`, `approve_create`, `approve_existing` và `reject`. Mỗi
lần sửa node chạy lại matching và validation edge liên quan. Apply chỉ ghi record
đã approve và trả `409 DATASET_VERSION_CONFLICT` nếu dataset đã thay đổi.
`revalidate` cập nhật schema/ontology/dataset hash, chạy lại matching và đưa toàn
bộ decision trong job về `pending` để admin duyệt lại an toàn.

Node properties đã duyệt được ghi vào `properties.csv` cùng provenance. Trải
nghiệm nổi bật cấp Place/Activity dùng property `special_experience` dạng JSON
array; Area không bắt buộc có property này.
Edge có ID ổn định và trường `recommendations` là JSON array; mỗi phần tử có thể chứa
`intent`, `priority`, `timeSlots`, `recommendedItems` và `reason`. Contract edge
không có trường `note`. Cạnh cấu trúc `LOCATED_IN` và `PART_OF` mặc định dùng
`recommendations=[]`.

Node type validation resolves inheritance declared in `schema.yaml`. Abstract
types (`Entity`, `LocationEntity`, `Place`) may appear in
ontology relationship contracts but cannot be persisted as entity types.
`Activity` is a concrete type connected from a Place by `OFFERS_ACTIVITY`.
