...
Dataset của Admin Knowledge Graph tại `knowledge-graph-real-v2` được bind mount
vào backend container ở cùng đường dẫn `/knowledge-graph-real-v2`. Vì vậy thao
tác duyệt/sửa graph trong container ghi trực tiếp vào dataset trên host, không tạo
một bản sao riêng bên trong image.
