...
`backend/app/modules/plans/knowledge_graph/hanoi_graph.v1.json` và traversal cục bộ.
...
trước khi semantic retrieval. Graph cũng cấp `diversityGroup` để tránh lặp cùng
trải nghiệm trong ngày. Embedding vẫn lấy Place candidate; rating, review, khoảng
cách và feasibility vẫn rerank/kiểm tra sau đó.
...
catalog PostgreSQL và được nối động với experience bằng metadata/embedding. Chỉ
