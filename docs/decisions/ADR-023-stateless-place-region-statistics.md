...
catalog `places`, tạo thêm state và quy trình refresh phải đồng bộ.

## Quyết định

- Xóa `place_region_snapshots` và `place_region_catalog_state`.
- Tính region metrics trực tiếp từ Place thuộc `region_key` được yêu cầu và các
  vùng con.
...
- Planner luôn nhìn thấy dữ liệu Place hiện tại của đúng vùng.
