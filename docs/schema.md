...
Information Finder đã có repository nguồn riêng. Module `conversation_memory` đã hoàn thành Phase 01 & Phase 02: bổ sung `MemoryRepository` port (`ports.py`), PostgreSQL asyncpg adapter (`adapters/postgres.py`), database migration `009_conversation_memory.sql` (idempotent, hỗ trợ backfill `normalized_value`, cột `source_url` và repair dữ liệu legacy), optimistic concurrency control (`version`), effective_expected_version resolution trong service, deduplication facts theo `chat_id + key + normalized_value`. Phase 02 đã bổ sung `FactExtractor` và `ReferenceResolver` ports, `RuleBasedFactExtractor` (trích xuất destination, duration, travelers, budget, place candidates, URL hỗ trợ tiếng Việt có/không dấu và viết tắt), `RuleBasedReferenceResolver` (phân giải anaphora, deictic, plan references với cơ chế clarification phòng ngừa hallucination), `MergePolicyEvaluator` bảo vệ confirmed facts (explicit user change có ưu tiên cao nhất) và duy trì lịch sử superseded facts. Cơ chế lưu trữ atomic `save_memory_and_facts` đảm bảo làm tươi state projection và facts trong cùng một transaction. APIs `extract_facts`, `resolve_references`, `merge_extracted_facts` và `process_message` đã khả dụng qua `public.py`. Tích hợp trực tiếp vào Root Graph runtime được giữ cho Phase 03.
