...
async def run_evaluation() -> dict[str, Any]:
    ...
    try:
        with Session(engine) as session:
            ...
            # Current PlaceSelector keeps meal venues in meal anchors and
            # fills only the capacity available for main activities.
            assert 1 <= len(catalog_items) <= 2
            assert all(item.source == "finder_suggestion" for item in catalog_items)
            assert "Accessible Museum" in {item.name for item in catalog_items}
...
class DeterministicPlannerLLM:
    ...
    async def generate_json(self, system_prompt: str, user_payload: str) -> str:
        ...
        catalog = envelope.get("graphCandidateCatalog", {}).get("candidates", [])
        required_experiences = []
        for index, candidate in enumerate(catalog[: trip_spec["days"]]):
            anchor_ids = candidate.get("anchorPlaceIds", [])
            candidate_ids = candidate.get("candidatePlaceIds", [])
            ...
            if anchor_ids:
                ...
                policy = "open_candidate"
            required_experiences.append(
                {
                    "requirementId": f"deterministic-{index + 1}",
                    "theme": candidate.get("activityName") or candidate.get("category", "experience"),
                    "category": candidate.get("category", "main_experience"),
                    "activityId": activity_id,
                    "selectionPolicy": policy,
                    "anchorPlaceIds": anchor_ids if policy == "required_anchor" else [],
                    "candidatePlaceIds": candidate_ids if policy == "choose_one" else [],
                    "minimumRequired": 1,
                    "priority": "must",
                    "reason": "Selected from the bounded graph candidate catalog.",
                    "evidenceClaimIds": candidate.get("claimIds", []),
                    "sourceRefs": candidate.get("sourceRefs", []),
                }
            )
        return json.dumps(
            {
    ...
    async def generate_structured_json(
        ...
        *,
        ...
        """Implement the current LLMClient contract for planner evaluation."""
        del response_schema
