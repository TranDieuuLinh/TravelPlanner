...
class BackupPlanWorkflow:
    ...
    async def run(self, main_plan: Plan, payload: BackupPlanCreate) -> tuple[Plan, CheckReport]:
        ...
        selection_output = self.place_selector.fill_agent_plan(
            PlaceSelectionInput(
                ...
                tripSpec=trip_spec,
                regionKey=normalize_region_key(main_plan.destination),
                tripThemes=main_plan.trip_themes,
                selectedPlaces=selected_places,
                userStatus=main_plan.initial_user_status,
            )
        )
        unscheduled_places = list(selection_output.unscheduled_places)
        backup_plan = Plan(
            id=str(uuid4()),
            ...
            planningAssumptions=[payload.reason],
            warnings=[
    ...
    def _selected_place_is_outdoor(
        ...
        if markers.intersection(tag.casefold() for tag in place.tags):
            return True
        if place.place_id:
            stored_place = self.place_selector.place_tool.get(place.place_id)
