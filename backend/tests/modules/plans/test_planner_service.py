...
def test_trip_theme_planner_uses_snapshot_and_returns_only_themes() -> None:
    ...
    assert not any(note.startswith("researchPromptVersion=") for note in output.trace.notes)
    assert "promptVersion=trip_theme_planner_graph_v2" in output.trace.notes
...
def test_planner_sends_small_area_statistics_to_llm() -> None:
    ...
    assert payload["promptVersion"] == "trip_theme_planner_graph_v2"
    assert payload["plannerInput"]["regionContext"]["plannerSignals"][
        "candidateAreas"
    ][0]["regionKey"] == "vn,ha-noi,hoan-kiem"
    assert payload["themeSelectionPolicy"]["selectionMode"] == (
        "current_trip_intent"
    )
...
def test_place_selector_reports_confirmed_places_over_day_capacity() -> None:
    ...
    allocated = sum(
        item.source == "selected_place"
        for day in plan.days
        for item in day.items
    )
    assert allocated == 4
    assert {item.place_id for item in plan.unscheduled_places} == {
        "place-5",
        "place-6",
        "place-7",
    }
...
def test_plan_service_keeps_url_overflow_unscheduled_for_fixed_duration() -> None:
    ...
    assert all(item.source_refs == [source_url] for item in plan.unscheduled_places)
    represented_names = {
        item.name for day in plan.days for item in day.items
...
def test_url_coverage_guard_retains_a_place_omitted_by_downstream_planner() -> None:
    ...
    damaged = plan.model_copy(
        update={
            "days": [
                day.model_copy(update={"items": []}) for day in plan.days
            ],
            "unscheduled_places": [],
        }
    )
...
def test_main_workflow_accepts_planning_context() -> None:
    ...
    assert len(plan.days) == 2
    assert {
        item.place_id for day in plan.days for item in day.items
    } >= {"place-van-mieu", "place-ho-guom"}
    assert plan.status.value == "draft"
    assert plan.check_report is not None
    assert plan.check_report.status == "needs_backup"
