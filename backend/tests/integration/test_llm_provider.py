...
def test_gemini_round_robins_keys_across_successful_calls(monkeypatch) -> None:
    FakeAsyncClient.responses = [
    ...
    client = GeminiLLMClient(
        "key-api1,key-api2,key-api3",
        "test-model",
        min_interval_seconds=0,
    )

    async def run_calls() -> None:
        await client.generate_json("system", "payload-1")
        await client.generate_json("system", "payload-2")
        await client.generate_json("system", "payload-3")

    asyncio.run(run_calls())

    assert FakeAsyncClient.api_keys == ["key-api1", "key-api2", "key-api3"]
