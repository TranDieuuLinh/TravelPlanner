...
class SearchPlacesBatchMixin:
    async def search_many(
        ...
        if (
            not requests
            or provider_search is None
            or not self._batch_compatible(requests)
        ):
            return list(await asyncio.gather(*(self.search(item) for item in requests)))

        started_at = time.perf_counter()
        name_batches = [
            lookup_names(request.query, request.alternate_names) for request in requests
        ]
        first = requests[0]
        try:
            ...
            return [
                self._provider_failure(request, names, started_at, exc)
                for request, names in zip(requests, name_batches)
            ]

        results = []
        for request, names, candidates in zip(
            requests, name_batches, candidate_batches
        ):
    ...
    def _batch_compatible(self, requests: list[PlaceSearchRequest]) -> bool:
        first = requests[0]
        if first.search_mode == "named_place":
            return False
        return all(
