...
def split_trip_preferences(
    ...
    for value in preferences:
        ...
        target = (
            styles
            if is_prefixed_style or normalized in KNOWN_TRIP_STYLES
            else tags
        )
        if normalized and normalized not in target:
            target.append(normalized)
    avoid_tags = list(
        dict.fromkeys(normalize_label(value) for value in avoids if normalize_label(value))
    )
...
def candidate_semantics(
    ...
    style_values = (
        relation.related_name or relation.related_entity_id or relation.to_entity_id
        for relation in relationships
        if relation.relationship_type == "Has_Style" and not relation.is_pending
    )
    styles = list(
        dict.fromkeys(
            normalized
            for value in style_values
            if (normalized := normalize_label(value.removeprefix("style:")))
        )
    )
    return semantic_tags, styles
...
def audience_values(
    *, adults: bool | None, children: bool | None, infants: bool | None
    ...
    if children is True or infants is True:
        ...
        kid_suitable = None
    adult_only = True if adults is True and kid_suitable is False else (
        False if kid_suitable is True else None
    )
